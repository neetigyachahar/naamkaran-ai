...
__export(index_exports, {
  analyzeName: () => analyzeName2,
  analyzeNameStream: () => analyzeNameStream,
  generateNamesHttp: () => generateNamesHttp,
  smartPickStream: () => smartPickStream
});
...
var NameGenreIdSchema = external_exports.enum([
  "best-fit",
  "action-phrase",
  "gen-z",
  "single-word",
...
var DEFAULT_NAME_COUNT = 10;
var GenerateNamesRequestSchema = external_exports.object({
  genreId: NameGenreIdSchema,
  messages: external_exports.array(ChatMessageSchema).min(1).max(30),
  context: external_exports.preprocess(
...
// ../../packages/shared/src/smart-pick.ts
...
// ../../packages/shared/src/analyze-stream.ts
var AnalyzeStreamRequestSchema = external_exports.object({
  name: external_exports.string().min(1).max(100),
  category: external_exports.preprocess(
    (value) => value === null || value === "" ? void 0 : value,
...
var DomainResultSchema = external_exports.object({
  ...
  source: external_exports.enum(["rdap", "whois"]),
  tier: external_exports.union([external_exports.literal(1), external_exports.literal(2), external_exports.literal(3)])
});
var DomainCheckResultSchema = external_exports.object({
  score: external_exports.number().min(0).max(100),
  results: external_exports.array(DomainResultSchema)
});
var SeoSourceSchema = external_exports.object({
  title: external_exports.string(),
  uri: external_exports.string()
});
var SeoResultSchema = external_exports.object({
  score: external_exports.number().min(0).max(100),
  isExistingBrand: external_exports.boolean(),
  confidence: external_exports.number().min(0).max(100),
  summary: external_exports.string(),
  sources: external_exports.array(SeoSourceSchema)
});
var McaMatchSchema = external_exports.object({
...
var ACTIVE_SCORE_WEIGHTS = REGISTRATION_CHECK_ENABLED ? SCORE_WEIGHTS : { domain: 0.5, seo: 0.5, registration: 0 };
var CACHE_TTL_MS = 7 * 24 * 60 * 60 * 1e3;
var COMPANY_MASTER_DATA_RESOURCE_ID = "4dbe5667-7b6b-41d7-82af-211562424d9a";
var TRADEMARK_SEARCH_BASE_URL = "https://tmrsearch.ipindia.gov.in/ESEARCH";

// src/lib/analysis-cache.ts
async function getCachedAnalysis(cacheKey) {
  try {
    const cacheRef = (0, import_firestore.getFirestore)().collection("nameAnalysisCache").doc(cacheKey);
...
async function setCachedAnalysis(cacheKey, result) {
  try {
    const cacheRef = (0, import_firestore.getFirestore)().collection("nameAnalysisCache").doc(cacheKey);
    await cacheRef.set({
      result,
      expiresAt: import_firestore.Timestamp.fromMillis(Date.now() + CACHE_TTL_MS)
    });
  } catch {
  }
}

// src/lib/rdap-bootstrap.ts
var IANA_RDAP_DNS_URL = "https://data.iana.org/rdap/dns.json";
...
async function getRdapBaseUrl(tld) {
  ...
  const now = Date.now();
  if (!cachedMap || now - cachedAt > CACHE_TTL_MS2) {
...
function queryWhois(server, domain, timeoutMs = 1e4) {
...
// src/modules/domain-check.ts
var CONCURRENCY = 5;
var UNKNOWN_WEIGHT_FACTOR = 0.5;
...
async function checkTld(name, tld, tier) {
...
function computeDomainScore(results) {
  let earned = 0;
  let total = 0;
  for (const result of results) {
...
async function domainCheck(name) {
  ...
  const results = await runWithConcurrency(
    TLDS,
    CONCURRENCY,
    (config) => checkTld(normalized, config.tld, config.tier)
  );
  return {
...
async function registrationCheck(name, apiKey, category) {
  ...
  for (let i = 0; i < variantSpecs.length; i++) {
    ...
    if (i > 0) await sleep2(REQUEST_GAP_MS);
...
async function analyzeName(name, secrets2, category, modelId, brandSearchMode = "lite") {
  ...
  const [domain, seo] = await Promise.all([
    ...
    seoCheck(brandName, secrets2.openRouterKey, category, model, brandSearchMode)
  ]);
...
async function analyzeNameWithProgress(name, secrets2, category, onProgress, modelId, options) {
  ...
  if (cached) {
    ...
    const skipSeo2 = options?.skipSeoIfDomainBelow != null && cached.domain.score < options.skipSeoIfDomainBelow;
    if (!skipSeo2) {
...
// src/config/name-genres.ts
var GENRE_INSTRUCTIONS = {
  "best-fit": {
...
function getGenreInstruction(genreId) {
  return GENRE_INSTRUCTIONS[genreId].system;
}
function resolveNameCount(messages) {
  const lastUser = [...messages].reverse().find((m) => m.role === "user");
  if (!lastUser) return DEFAULT_NAME_COUNT;
  const text = lastUser.content.toLowerCase();
  const explicitCount = text.match(/\b(?:give\s+me|need|want|show|list|only|just)\s+(\d+)\s*names?\b/) ?? text.match(/\b(\d+)\s*names?\s*(?:only|please)?\b/) ?? text.match(/\b(?:only|just)\s+(\d+)\b/);
  if (explicitCount) {
...
// src/modules/name-generation.ts
function buildSystemPrompt(genreId, nameCount, context, smartPick, excludeNames) {
...
- Generate a diverse candidate pool (${nameCount} names) \u2014 variety matters since many will be filtered out.` : "";
  const excludeNote = excludeNames && excludeNames.length > 0 ? `
- Do NOT repeat these already-tried names: ${excludeNames.join(", ")}` : "";
  const countRule = nameCount < DEFAULT_NAME_COUNT ? `Suggest exactly ${nameCount} names (user requested fewer this turn).` : `Suggest at least ${nameCount} names \u2014 never fewer unless the user explicitly asked for a smaller number this turn.`;
...
function parseResponse(text, minCount) {
  const cleaned = text.replace(/```json\s*/gi, "").replace(/```\s*/g, "").trim();
  const jsonMatch = cleaned.match(/\{[\s\S]*\}/);
  const parsed = JSON.parse(jsonMatch?.[0] ?? cleaned);
  const names = (parsed.names ?? []).map((n) => String(n).trim()).filter((n) => n.length > 0 && n.length <= 50).slice(0, Math.max(minCount, 20));
  return {
...
async function runSmartPickStream(genreId, messages, apiKey, context, emit, modelId, dataGovKey2) {
  ...
  let replySent = false;
  let candidateQueue = [];
  while (tried.size < SMART_PICK_MAX_CANDIDATES && accepted.length < SMART_PICK_REVEAL_COUNT) {
    if (candidateQueue.length === 0) {
      const shouldGenerateMore = tried.size === 0 || accepted.length < SMART_PICK_MIN_ACCEPTED || accepted.length < SMART_PICK_REVEAL_COUNT;
      ...
      candidateQueue = batch.names.map((name2) => name2.trim()).filter((name2) => name2.length > 0 && !tried.has(name2));
      if (candidateQueue.length === 0) {
    ...
    try {
      const result = await analyzeNameWithProgress(
        name,
        { openRouterKey: apiKey, dataGovKey: dataGovKey2 },
        context,
        (step) => {
          if (step.type === "domain_start") {
            emit({ type: "domain_check", name, status: "start" });
      ...
      const message2 = error instanceof Error ? error.message : "Analysis failed";
      emit({ type: "error", message: `${name}: ${message2}` });
    }
  }
  const message = accepted.length < SMART_PICK_MIN_ACCEPTED ? `Found ${accepted.length} strong name${accepted.length === 1 ? "" : "s"} (target was ${SMART_PICK_MIN_ACCEPTED}+ at ${SMART_PICK_MIN_SCORE}+).` : void 0;
  const sortByScore = (names) => [...names].sort((a, b) => (nameScores[b] ?? 0) - (nameScores[a] ?? 0));
  emit({
    type: "done",
...
var analyzeName2 = (0, import_https.onCall)(
  ...
  async (request) => {
    ...
    const cat = category?.trim().toLowerCase() || "";
    const cacheKey = cat ? `${analysisCacheKey(name, resolvedModel, brandSearchMode)}:cat:${cat}` : analysisCacheKey(name, resolvedModel, brandSearchMode);
    const cached = await getCachedAnalysis(cacheKey);
...
var analyzeNameStream = (0, import_https.onRequest)(
  {
    ...PUBLIC_CORS_OPTIONS,
    secrets,
    timeoutSeconds: 180,
